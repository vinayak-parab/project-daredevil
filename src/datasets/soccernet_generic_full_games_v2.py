...
class soccernet_dataset_generic(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        ms[ms.isneginf()] = 0
        resnet_audio_feature = self.resnet_audio.get(vidpath)[half - 1]
        # normalize
        ...
        sample = {'vidpath': vidpath,
                  ...
                  'resnet_audio_feature': resnet_audio_feature,
    ...
    def load_audio_resnet(self):
        if self.lang_sub_samples:
            for path in tqdm(self.lang_sub_samples):
                half1 = np.load(self.root_dir+path+"/resnet18_audio_feat_v1_h1.npy")
                half2 = np.load(self.root_dir+path+"/resnet18_audio_feat_v1_h2.npy")
                self.resnet_audio[path] = (half1,half2)
        else:
            print("Already loaded!")
